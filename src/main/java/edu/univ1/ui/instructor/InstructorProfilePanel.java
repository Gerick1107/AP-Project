...
public class InstructorProfilePanel extends JFrame{
    private final User user;
    private final InstructorAPI instructorAPI=new InstructorAPI();
    public InstructorProfilePanel(User user){
        this.user=user;
        setTitle("My Profile");
        setSize(550,420);
        ...
        Instructor instructor=instructorAPI.getProfile(user.getUserId());
        if(instructor==null){
            UIUtils.error("Unable to load profile.");
            dispose();
            return;
        }
        JPanel card=new JPanel(new MigLayout("wrap 2","[grow][grow]","12[]12[]12"));
        ...
        JLabel v2=new JLabel(department!=null&&!department.isBlank()?department:"-");
